...
from bragg_peak import bragg_peak
from bragg_peak import random_bragg
...
class Source:
    ...
    def produce_alpha(self,n,store,ionization_profile,phi_in=None,ath_in=None,theta=None): 
        ...
        if ionization_profile=='Bragg':
            self.x,self.Sp=bragg_peak(self.energy,self.range_alpha)
        else:
            self.x=None
            self.Sp=None
        ...
        for i in range(n):
            ...
            alpha=Alphas_tracks(x=self.x,Sp=self.Sp,range_alpha=self.range_alpha,phi=phi,ath=ath,x0=x0,y0=y0,ionization_profile=ionization_profile)
            store.append(alpha)
...
class Alphas_tracks:
    ...
    def __init__(self,x,Sp,range_alpha,phi=None,ath=None,x0=None,y0=None,spread=0,ionization_profile="Flat"):
        
        self.ionization=ionization_profile
        self.X=x
        self.Sp=Sp  #si no da error
    ...
    def fill(self):
        ...
        for i in range(self.n_electrons):
            ...
            if self.ionization=='Bragg':
                r_pos=self.ionization_profile(self.X,self.Sp)*self.range
            else:
