...
pcut1_lower = 1450 #0.3, 0.8, ~1.3, 2.5, 8
pcut1_upper = 1500
...
# INFINITE GAIN
plt.figure()
plt.title('Gain = $\infty$', fontsize = 30)
...
plt.xlabel('P (MeV/c)', fontsize = 30 )
plt.ylabel('dE/dx (keV/cm)', fontsize = 30 )
...
#Saving data
"""
MuonCut_tosave = np.array(Cut1_muon['dEdx_trunc'])
PionCut_tosave =np.array(Cut1_pion['dEdx_trunc'])
ElectronCut_tosave =np.array(Cut1_electron['dEdx_trunc'])

np.savetxt('MuonData_inf_'+cut, MuonCut_tosave)
np.savetxt('PionData_inf_'+cut, PionCut_tosave)
np.savetxt('ElectronData_inf_'+cut, ElectronCut_tosave)
"""
...
smearing = np.sqrt(dEdx_res_g10000**2 - (optMuon[2]/optMuon[1]*100)**2)
...
# GAIN
plt.figure()
plt.title('Gain = {}'.format(gain), fontsize = 30)
...
#DOING THE CUT
Cut2 = data_final[(data_final['P_good'] > pcut1_lower) & (data_final['P_good'] < pcut1_upper)]
#Cut in P
Cut2_muon = Cut2[Cut2['ID'] == 0]
Cut2_pion = Cut2[Cut2['ID'] == 1]
Cut2_kaon = Cut2[Cut2['ID'] == 2]
Cut2_proton = Cut2[Cut2['ID'] == 3]
Cut2_electron = Cut2[Cut2['ID'] == 4]
...
axs[0].plot(Cut2['P_good'], Cut2['dEdx_trunc_sm'], 'ko', alpha = 0.3)
axs[0].set_xlabel('P (MeV/c)', fontsize = fontsize_subaxis)
axs[0].set_ylabel('dE/dx (keV/cm)', fontsize = fontsize_subaxis)
axs[0].tick_params(axis='both', which='major', labelsize=fontsize_legend)

#axs[1].hist(Cut2['dEdx_trunc_sm'], bins = bins2, color='k', histtype = 'step', label = 'total', zorder = 3, linewidth = 1.5)
MuonCutData = axs[1].hist(Cut2_muon['dEdx_trunc_sm'], bins = bins1, histtype = 'step', label = 'muon', color = 'b')
PionCutData = axs[1].hist(Cut2_pion['dEdx_trunc_sm'], bins = bins1, histtype = 'step', label = 'pion', color ='g')
KaonCutData = axs[1].hist(Cut2_kaon['dEdx_trunc_sm'], bins = bins1, histtype = 'step', label = 'kaon', color = 'r')
ProtonCutData = axs[1].hist(Cut2_proton['dEdx_trunc_sm'], bins = bins1, histtype = 'step', label = 'proton', color ='cyan')
ElectronCutData = axs[1].hist(Cut2_electron['dEdx_trunc_sm'], bins = bins1, histtype = 'step', label = 'electron', color = 'orange')
axs[1].set_xlabel('dEdx (keV/cm)', fontsize = fontsize_subaxis)
axs[1].set_ylabel('Counts', fontsize = fontsize_subaxis)
...
#Saving data
"""
MuonCut_tosave = np.array(Cut2_muon['dEdx_trunc_sm'])
PionCut_tosave =np.array(Cut2_pion['dEdx_trunc_sm'])
ElectronCut_tosave =np.array(Cut2_electron['dEdx_trunc_sm'])

np.savetxt('MuonDat'+gain+'_'+cut, MuonCut_tosave)
np.savetxt('PionData'+gain+'_'+cut, PionCut_tosave)
np.savetxt('ElectronData'+gain+'_'+cut, ElectronCut_tosave)
"""
# dEdx RESOLUTION =============================================================
def gaussian(x, A, mu, sigma):
    return A*np.exp(-(x-mu)**2/(2*sigma**2))
